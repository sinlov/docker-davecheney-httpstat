...
INFO_TEST_BUILD_PARENT_CONTAINNER ?= test-parent-docker-davecheney-httpstat
INFO_TEST_TAG_BUILD_IMAGE_NAME ?= sinlov/docker-davecheney-httpstat
INFO_TEST_TAG_BUILD_CONTAINER_NAME ?= test-docker-davecheney-httpstat

env:
	@echo "====== show build evn start" =====
	@echo ""
	@echo "ROOT_SWITCH_TAG                ${ROOT_SWITCH_TAG}"
	@echo ""
...
dockerRemoveBuild:
	-docker rmi -f $(INFO_TEST_TAG_BUILD_IMAGE_NAME):test-$(ROOT_SWITCH_TAG)

dockerBuild:
	docker build -t $(INFO_TEST_TAG_BUILD_IMAGE_NAME):test-$(ROOT_SWITCH_TAG) .
	docker run --rm --name ${INFO_TEST_TAG_BUILD_CONTAINER_NAME} $(INFO_TEST_TAG_BUILD_IMAGE_NAME):test-$(ROOT_SWITCH_TAG) --help
